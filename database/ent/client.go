...
func (c *DialogClient) MapCreateBulk(slice any, setFunc func(*DialogCreate, int)) *DialogCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &DialogCreateBulk{err: fmt.Errorf("calling to DialogClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*DialogCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *DialogClient) UpdateOneID(id int64) *DialogUpdateOne {
...
func (c *DialogClient) DeleteOneID(id int64) *DialogDeleteOne {
...
func (c *DialogClient) Get(ctx context.Context, id int64) (*Dialog, error) {
	return c.Query().Where(dialog.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *DialogClient) GetX(ctx context.Context, id int64) *Dialog {
...
// QueryMessages queries the messages edge of a Dialog.
func (c *DialogClient) QueryMessages(d *Dialog) *MessageQuery {
	query := (&MessageClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := d.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(dialog.Table, dialog.FieldID, id),
...
func (c *MessageClient) QueryDialog(m *Message) *DialogQuery {
	query := (&DialogClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Dialog, Message []ent.Hook
	}
	inters struct {
		Dialog, Message []ent.Interceptor
	}
)
