...
func (mq *MessageQuery) QueryDialog() *DialogQuery {
	query := (&DialogClient{config: mq.config}).Query()
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withDialog: mq.withDialog.Clone(),
...
// WithDialog tells the query-builder to eager-load the nodes that are connected to
// the "dialog" edge. The optional arguments are used to configure the query builder of the edge.
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withDialog; query != nil {
		if err := mq.loadDialog(ctx, query, nodes, nil,
			func(n *Message, e *Dialog) { n.Edges.Dialog = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadDialog(ctx context.Context, query *DialogQuery, nodes []*Message, init func(*Message), assign func(*Message, *Dialog)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "dialog_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withDialog != nil {
			_spec.Node.AddColumnOnce(message.FieldDialogID)
		}
	}
